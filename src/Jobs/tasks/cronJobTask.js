      ...
      if (!result.isError) {
        if (monitor.status === "down" || monitor.status === "uncertain") {
          try {
            await emailService.sendEmailHtml(
                          ...
                          `Monitor is UP ${monitor.monitorType}  URL: ${cronJobMonitor.host}`,
                          `
            <!doctype html>
            <html lang="tr">
            <head>
        ...
        if (monitor.failCount === 0) {
          try {
            await emailService.sendEmailHtml(
                          `<${monitor.serverOwner.email}>`,
                          `Monitor is DOWN. ${monitor.monitorType} on ${cronJobMonitor.host} ${cronJobMonitor.method}`,
                          `
            <!doctype html>
            <html lang="tr">
            <head>
