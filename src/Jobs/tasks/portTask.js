    ...
    if (monitor.isActiveByOwner && !flag) {
      ...
      if (!result.isError) {
        if (
          ...
          (result.slowResponse.isError && portMonitor.slowResponseAlertStatus)
        ) {
          try {
            await emailService.sendEmailHtml(
                          `<${monitor.serverOwner.email}>`,
                          `Monitor is UP ${monitor.monitorType} on ${portMonitor.host} ${portMonitor.port}`,
                          `
            <!doctype html>
            <html lang="tr">
            <head>
                            ...
                            <tr>
                              <td style="padding:8px 0;">
                                ...
                                <div style="font-weight:700;margin-top:6px;">${
                                  monitor.monitorType
                                } — ${portMonitor.host} Port: ${portMonitor.port}</div>
                              </td>
                            </tr>
                          </table>
        ...
        if (
          ...
          (result.slowResponse.isError &&
            monitor.portMonitor.slowResponseAlertStatus)
        ) {
          try {
            await emailService.sendEmailHtml(
                          `<${monitor.serverOwner.email}>`,
                          `Monitor is DOWN. ${monitor.monitorType} on Host: ${portMonitor.host}  Port: ${portMonitor.port}`,
                          `
            <!doctype html>
            <html lang="tr">
            <head>
