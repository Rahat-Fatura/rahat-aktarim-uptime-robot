    ...
    if (monitor.isActiveByOwner && !flag) {
      ...
      if (!result.isError) {
        if (
          ...
          (result.slowResponse.isError && pingMonitor.slowResponseAlertStatus)
        ) {
          try {
            await emailService.sendEmailHtml(
              `<${monitor.serverOwner.email}>`,
              `Monitor is UP. ${monitor.monitorType} on Host: ${pingMonitor.host}`,
              `
<!doctype html>
<html lang="tr">
<head>
...
`
        ...
        if (
          ...
          (result.slowResponse.isError &&
            monitor.pingMonitor.slowResponseAlertStatus)
        ) {
          try {
            await emailService.sendEmailHtml(
              `<${monitor.serverOwner.email}>`,
              `Monitor is DOWN. ${monitor.monitorType} on Host: ${pingMonitor.host}`,
              `
            <!doctype html>
            <html lang="tr">
            <head>
