...
const {
  cronJobMonitorTask,
} = require("../Jobs/queuesWorker/cronJobMonitorTask");

cronJobMonitorTask();

const createMonitor = catchAsync(async (req, res) => {
  const monitor = await cronJobMonitorService.createCronJobMonitor(
    req.body,
    req.user.id
  );
...
const getMonitor = catchAsync(async (req, res) => {
  const monitor = await cronJobMonitorService.getCronJobMonitorFullDataById(
    req.params.id
  );
...
const updateMonitor = catchAsync(async (req, res) => {
  ...
  let monitor = await cronJobMonitorService.getCronJobMonitorById(
    req.params.id
  );
  if (!monitorBody || !monitor) {
    return res
      .status(httpStatus.NOT_FOUND)
      .send({ message: "Monitor not found" });
  }
  if (
    updateData.interval !== monitorBody.interval ||
    updateData.intervalUnit !== monitorBody.intervalUnit ||
    updateData.cronJobMonitor.devitionTime !== monitor.devitionTime
  ) {
  ...
  monitor = await cronJobMonitorService.updateCronJobMonitorById(
    req.params.id,
    updateData
  );
  res.status(httpStatus.OK).send(monitor);
