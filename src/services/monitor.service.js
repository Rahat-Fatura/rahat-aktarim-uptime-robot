...
const getMonitorForEdit = async (id) => {
  const monitor = await Monitor.findUnique({
    where: { id: Number(id) },
    include: {
      httpMonitor: true,
...
const getMonitorWithBodyForTask = async (id) => {
  let monitor;
  try {
    monitor = await Monitor.findFirst({
      where: {
        AND:[
          {id: Number(id)},
          {status:{not: "maintanance"}}
        ]
      },
      select: {
        id: true,
